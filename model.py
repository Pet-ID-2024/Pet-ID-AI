...
class MultiOutputModel(nn.Module):
    def __init__(self, n_breed_classes, n_hair_classes):
        super().__init__()
        self.base_model = models.mobilenet_v3_large(pretrained=True, progress=True).features  # take the model without classifier
        last_channel = 960
    ...
    def get_loss(self, net_output, ground_truth):
        ...
        # print(net_output['breed'])
        # print(ground_truth['breed_labels'])
        # print(net_output['hair'].shape)
        breed_loss = criterion(torch.max(net_output['breed']), ground_truth['breed_labels'])
        hair_loss = criterion(torch.max(net_output['hair']), ground_truth['hair_labels'])
